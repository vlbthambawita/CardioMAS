...
class SplitDataMissingError(RuntimeError):
    """Raised when no executor-generated splits are available (V4)."""
...
def splitter_agent(state: GraphState) -> GraphState:
    ...
    # ── Load record IDs — V4 executor output > Phase 2 DatasetMap > CSV fallback
...
def _load_record_ids(state: GraphState, analysis: dict) -> list[str]:
    """Load real record IDs — V4 executor > DatasetMap > CSV fallback.

    Priority:
    1. V4: splits.json generated by 04_generate_splits_full.py
    2. V3: DatasetMap (Phase 2)
    3. V3: CSV file scan (legacy fallback)
    """

    # 0. V4: executor-generated splits.json (highest priority)
    v4_splits_json = state.v4_generated_files.get("splits.json", "")
    if v4_splits_json:
        try:
            data = json.loads(v4_splits_json)
            ...
            if isinstance(splits, dict):
                ...
                if unique_ids:
                    vprint("splitter", f"loaded {len(unique_ids)} IDs from V4 splits.json")
                    return unique_ids
        except json.JSONDecodeError as exc:
            logger.warning(f"splitter: could not parse V4 splits.json: {exc}")

    # Also check splits_subset.json as fallback for subset runs
    v4_subset_json = state.v4_generated_files.get("splits_subset.json", "")
    if v4_subset_json and not v4_splits_json:
        try:
            data = json.loads(v4_subset_json)
            splits = data.get("splits", data)
            if isinstance(splits, dict):
                all_ids = []
                for split_ids in splits.values():
                ...
                unique_ids = sorted(set(all_ids))
                if unique_ids:
                    vprint("splitter", f"loaded {len(unique_ids)} IDs from V4 splits_subset.json")
                    return unique_ids
