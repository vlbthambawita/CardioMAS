...
# ---------------------------------------------------------------------------
# Standalone script support (script_only dataset_mode)
# ---------------------------------------------------------------------------
...
def _script_name(task: str, dataset_path: str) -> str:
    digest = hashlib.sha1(f"{task}|{dataset_path}".encode("utf-8")).hexdigest()[:8]
    base = _slug(task or "analysis")[:40] or "analysis"
    return f"{base}_{digest}.py"


def _render_standalone_script_source(
    task: str,
    dataset_path: str,
    output_dir: Path,
    mode: str,
    ...
    error_comment = f"# Previous error: {last_error}\n" if last_error else ""
...
OUTPUT:
    Results printed to stdout and written to:
...
OUTPUT_DIR   = Path({output_literal})
TASK         = {task_literal}
MODE         = {mode_literal}
...
SKIP_FILENAMES   = {{"license.txt", "sha256sums.txt"}}
ID_COLUMN_HINTS  = {{
    "patient", "patient_id", "patientid", "subject", "subject_id", "subjectid",
    "id", "ecg_id", "ecgid", "record", "record_id", "recordid",
}}
...
def main():
    ...
    if not DATASET_PATH:
        _write_and_print({{"ok": False, "summary": "DATASET_PATH is not configured.", "error": "dataset_path missing"}})
        return
    if not root.exists():
        _write_and_print({{"ok": False, "summary": f"Dataset path not found: {{root}}", "error": f"path not found: {{root}}"}})
        return

    try:
        if MODE == "inspect_directory":
            result = _summarize_directory(root)
        else:
            result = _summarize_dataset(root, TASK)
    except Exception as exc:
        _write_and_print({{"ok": False, "summary": f"Analysis failed: {{exc}}", "error": str(exc), "dataset_path": str(root)}})
        return

    result.update({{"task": TASK, "dataset_path": str(root), "ok": True}})
    _write_and_print(result)


def _write_and_print(result):
...
def _summarize_dataset(root, task):
    directory = _summarize_directory(root)
    files = _collect_files(root)
    tabular_files = [p for p in files if p.suffix.lower() in TABULAR_SUFFIXES]
    if tabular_files:
        selected = _best_scoring_file(tabular_files, root, task)
        table = _analyze_table(selected, root)
        directory.update(table)
        directory["mode"] = "analyze_dataset"
        directory["summary"] = (
            f"Analysis of `{{table['selected_path']}}`: "
            f"{{table.get('total_rows', 0)}} row(s), {{len(table.get('columns', []))}} column(s). "
            f"Dataset root has {{directory.get('total_files', 0)}} file(s)."
        )
        return directory
    selected = _select_target_file(root, "", task)
    if selected is not None:
        ...
        return result
    directory["mode"] = "analyze_dataset"
    directory["summary"] = "Inspected dataset directory but found no readable file."
    return directory
...
def _summarize_directory(root):
    ...
    return {{
        "mode": "inspect_directory",
        "summary": (
            f"Directory `{{root.name or root}}`: {{len(files)}} file(s) across "
            f"{{len(extension_counts)}} extension group(s)."
        ),
        "total_files": len(files),
        "extension_counts": dict(sorted(extension_counts.items())),
...
def _analyze_table(path, root):
    ...
    with path.open("r", encoding="utf-8", newline="") as handle:
        reader = csv.DictReader(handle, delimiter=delimiter)
        columns = list(reader.fieldnames or [])
        sample_rows, total_rows, missing_cells = [], 0, 0
        ...
        for row in reader:
            ...
            for col in columns:
                ...
                if not value:
                    ...
                    continue
                lower_col = col.lower()
                if lower_col in ID_COLUMN_HINTS or any(h in lower_col for h in ID_COLUMN_HINTS):
                    counter = candidate_counts.setdefault(col, Counter())
                    counter[value] += 1
                if lower_col in {{"label", "labels", "class", "classes", "target", "diagnosis", "split", "fold"}}:
                    counter = candidate_counts.setdefault(col, Counter())
                    counter[value] += 1
                number = _to_float(value)
                if number is not None:
                    numeric_values.setdefault(col, []).append(number)
            if len(sample_rows) < 5:
                sample_rows.append(normalized)

    unique_counts = {{col: len(c) for col, c in candidate_counts.items()}}
    numeric_summary = {{}}
    for col, vals in numeric_values.items():
        if not vals:
            continue
        numeric_summary[col] = {{"min": round(min(vals), 4), "max": round(max(vals), 4), "mean": round(sum(vals) / len(vals), 4)}}
        if len(numeric_summary) >= 5:
            break

    relative_path = str(path.relative_to(root))
    missing_fraction = round(missing_cells / float(total_rows * len(columns)), 4) if total_rows and columns else 0.0
    summary_parts = [f"Tabular file `{{relative_path}}`: {{total_rows}} row(s), {{len(columns)}} column(s), missing={{missing_fraction}}."]
    if unique_counts:
        parts = [f"`{{col}}`: {{n}} unique" for col, n in unique_counts.items()]
        summary_parts.append("Unique counts — " + "; ".join(parts) + ".")
    ...
    return {{
        ...
        "class_counts": {{col: dict(list(c.items())[:20]) for col, c in candidate_counts.items()}},
        "numeric_summary": numeric_summary,
    }}
...
def _analyze_json(path, root):
    ...
    if isinstance(payload, list):
        sample = payload[:3]
        keys = sorted(sample[0].keys()) if sample and isinstance(sample[0], dict) else []
        return {{"mode": "read_file", "summary": f"JSON list in `{{relative_path}}`: {{len(payload)}} item(s).",
                "selected_path": relative_path, "json_type": "list", "total_rows": len(payload),
                "columns": keys, "sample_rows": sample}}
    if isinstance(payload, dict):
        keys = sorted(payload.keys())
        return {{"mode": "read_file", "summary": f"JSON object in `{{relative_path}}`: {{len(keys)}} key(s).",
                "selected_path": relative_path, "json_type": "object", "columns": keys,
                "sample_rows": [{{k: payload[k] for k in keys[:10]}}]}}
    return {{"mode": "read_file", "summary": f"JSON scalar in `{{relative_path}}`.",
            "selected_path": relative_path, "json_type": type(payload).__name__, "sample_rows": [payload]}}
...
def _analyze_text(path, root):
    ...
    for line in preview:
        ...
        if len(fields) >= 12:
            break
    relative_path = str(path.relative_to(root))
    return {{"mode": "read_file", "summary": f"Text preview of `{{relative_path}}`: {{len(preview)}} line(s).",
            "selected_path": relative_path, "preview_lines": preview, "detected_fields": fields, "line_count": len(lines)}}


def _collect_files(root):
    return [p for p in sorted(root.rglob("*")) if p.is_file() and p.name.lower() not in SKIP_FILENAMES]


def _select_target_file(root, target_path, task):
    if target_path:
        c = Path(target_path) if Path(target_path).is_absolute() else root / target_path
        if c.exists() and c.is_file():
            return c
    files = _collect_files(root)
    return _best_scoring_file(files, root, task) if files else None


def _best_scoring_file(files, root, task):
    tokens = set(_tokenize(task))
    ranked = sorted(files, key=lambda p: (-_score_file(p, root, tokens), str(p)))
    best = ranked[0]
    if _score_file(best, root, tokens) <= 0:
        preferred = [p for p in files if p.suffix.lower() in TABULAR_SUFFIXES | JSON_SUFFIXES | TEXT_SUFFIXES]
        return preferred[0] if preferred else files[0]
    return best
...
def _score_file(path, root, tokens):
    relative = str(path.relative_to(root)).lower()
    score = sum(3 for t in tokens if t and t in relative)
    s = path.suffix.lower()
    score += 4 if s in TABULAR_SUFFIXES else (3 if s in JSON_SUFFIXES else (2 if s in TEXT_SUFFIXES else 0))
    if path.name.lower() in SKIP_FILENAMES:
        score -= 10
    return score
