...
def retrieve(chunks: list[EvidenceChunk], query: str, config: RetrievalConfig) -> list[EvidenceChunk]:
    if config.mode == "bm25":
        return _filter_min_score(retrieve_bm25(chunks, query, config.top_k), config.min_score)
    if config.mode == "dense":
        return _filter_min_score(retrieve_dense(chunks, query, config.top_k), config.min_score)

    bm25_hits = retrieve_bm25(chunks, query, max(config.top_k * 2, config.top_k))
    dense_hits = retrieve_dense(chunks, query, max(config.top_k * 2, config.top_k))
    fused: dict[str, float] = {}
    by_id: dict[str, EvidenceChunk] = {}

    for rank, hit in enumerate(bm25_hits):
        fused[hit.chunk_id] = fused.get(hit.chunk_id, 0.0) + 1.0 / (60 + rank + 1)
